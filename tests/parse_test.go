...
func TestParse(t *testing.T) {
	...
	if !ok || announce.Type != bencode.StringType || announce.Str != "http://bttracker.debian.org:6969/announce" {
		t.Fatalf("Expected announce URL to be 'http://tracker.example.com/announce', got %v", announce.Str)
	}

	// Check comment field
	comment, ok := result.Dict["comment"]
	if !ok || comment.Type != bencode.StringType || comment.Str != "\"Debian CD from cdimage.debian.org\"" {
	...
	if !ok || name.Type != bencode.StringType || name.Str != "debian-12.8.0-amd64-netinst.iso" {
